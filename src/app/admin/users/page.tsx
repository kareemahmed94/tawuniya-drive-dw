...
import { adminApi, type User } from '@/lib/admin/api-client';
...
export default function UsersPage() {
  ...
  const handleToggleStatus = async (user: User) => {
    try {
      setError(null);
      const response = await adminApi.toggleUserStatus(user.id, !user.isActive);
      if (response.success) {
        loadUsers();
      } else {
                      ...
                      <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {user.wallet ? (
                            <div>
                              <div className="text-sm font-medium text-gray-900">
                                {Number(user.wallet.balance).toLocaleString()} pts
                              </div>
                              <div className="text-xs text-gray-500">
                                Earned: {Number(user.wallet.totalEarned).toLocaleString()}
                              </div>
                            </div>
                          ) : (
      ...
      <div className="flex min-h-screen items-center justify-center px-4 py-4">
        ...
        <div className="relative inline-block w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {user ? 'New Password (leave empty to keep current)' : 'Password'} <span className="text-red-500">*</span>
              </label>
              <input
                type="password"
                required={!user}
